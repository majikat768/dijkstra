// _solve transforms our graph to a graph on NPM that solves our algorithm. This
// is temporary and only used for testing.
function _solve(start, end, ourGraph) {
  let route = new NaughtyGraph();
  for (let node of ourGraph.nodes) {
    route.addNode(node.id.toString(), new Map([
  ...
  return route.path(start.id.toString(), end.id.toString()).path;
}
